...
module.exports = (prDir, action, callback, opts) => {
  ...
  co(function* () {
    ...
    let projectConfDir = path.join(projectDir, '/config.js');
    ...
    let htmlBeautifyConf = {
      "indent_size": 4,
      "indent_char": " ",
      "eol": "\n"
    };

    let useTinypng = config.tinypng || false;

    /**
     * Configure proxy middleware
     */
    const proxyConfig = projectConf.devServer ? projectConf.devServer.proxy : undefined;
    ...
    gulp.task('images', function () {
      let imagesFilter = filter(['**/*.png', '**/*.jpg', '**/*.jpeg', '!**/__*.png', '!**/demo/*.*'], {restore: true});

      return gulp.src(projectDir + src + '/images/**')
        .pipe(debug({title: 'SLICE-IMAGES: ' + gutil.colors.green('✔')}))
        .pipe(imagesFilter)
        .pipe(gulpif(((opts.isIm || isDeploy) && !useTinypng), imagemin(imageminConf)))
        .pipe(gulpif(((opts.isIm || isDeploy) && useTinypng), tinypng()))
    ...
    gulp.task('sprite', ['css'], function () {
      let imagesFilter = filter(['**/*.png', '**/*.jpg', '**/*.jpeg'], {restore: true});
    ...
    // 删除.gulp目录
    gulp.task('build:del.gulp', function () {
      return del([path.join(pwd, './.gulp')], {force: true});
    });
