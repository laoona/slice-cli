...
module.exports = (prDir, action, callback, opts) => {
    ...
    co(function *() {
        ...
        let deployDir = [projectDir, './', '../../', './prod/', prBasename, '/'];
        let buildDir = [projectDir, './', 'build', '/'];

        console.log(buildDir, deployDir);
        // buildDir.unshift(projectDir);

        buildDir = path.join.apply(null, buildDir);
        deployDir = path.join.apply(null, deployDir);


        console.log(buildDir, deployDir, 'deploy');
        ...
        let smarty4jsConf = {
            ...
            rootDir: path.resolve(projectDir, smartyRootDir) || projectDir + '/templates/../../'
        };
        ...
        let spritesmithCfgDefault = {
            ...
            imagepath: spriteDir || path.join(prDir, src, '/dfimages/slice'),
        ...
        console.log(spritesmithCfgDefault);
        ...
        gulp.task('sprite', ['css'], function () {
            return gulp.src(projectDir + src + '/css/**/*.css')
                ...
                .pipe(gulpif((opts.isIm), imagemin(imageminConf)))
        ...
        gulp.task('sprite-fixed', ['sprite'], function () {
            ...
            return gulp.src(pathFixed)
                ...
                .pipe(gulp.dest(buildDir + src + '/css'))
                .on('end', function () {
                    console.log(buildDir+prDir, 'end');
                    del([buildDir + prDir]);
                });
        });
...
/*            return gulp.src(buildDir + '/!**!/!*')
        ...
        // build 任务列表
        let builds = ['images', taskHtmlType, 'build-templates', 'css', 'fonts', 'js', 'files'];
